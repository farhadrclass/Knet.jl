...
extern "C" { 
$DLLEXPORT void $(F)_21(int nx, $T *x, int sy, int ny, $T *y) {
  // x[i] goes into y[(i/sy)%ny]
  _$(F)_21<<<$BLK,$THR>>>(nx,x,sy,ny,y);
}
$DLLEXPORT void $(F)_21_stream(int nx, $T *x, int sy, int ny, $T *y, cudaStream_t STR) {
  // x[i] goes into y[(i/sy)%ny]
  _$(F)_21<<<$BLK,$THR,0,STR>>>(nx,x,sy,ny,y);
}
}
